...
parser.add_argument(
    ...
    action="store",
    dest="customquality",
    help="For configure quality of video.",
    default=0,
)
parser.add_argument(
    "--atmos",
    dest="atmos",
    help="If set, return atmos audio manifest",
    action="store_true",
)
parser.add_argument(
    "--only-2ch-audio",
    dest="only_2ch_audio",
    help="If set, to force get only eac3 2.0 Ch audios.",
    action="store_true",
)
parser.add_argument(
    "--hevc",
    dest="hevc",
    help="If set, return hevc video manifest",
    action="store_true",
)
parser.add_argument(
    "--hdr",
    dest="hdr",
    help="If set, return uhd_hdr video manifest",
    action="store_true",
)
parser.add_argument(
    "--uhd", dest="uhd", help="If set, return uhd video manifest", action="store_true"
)
parser.add_argument(
    "--default-audio-mux",
    action="store",
    dest="default_audio_mux",
    help="set default audio language mux, default value is eng.",
    default=0,
)
parser.add_argument(
    "--default-sub-mux",
    action="store",
    dest="default_sub_mux",
    help="set default sub language mux, default value is eng.",
    default=0,
)
parser.add_argument(
    "--all-season",
    dest="all_season",
    help="If set, season pack download.",
    action="store_true",
)
parser.add_argument(
    "-e",
    "--episode",
    dest="episode",
    help="If set, it will start downloading the season from that episode.",
)
parser.add_argument(
    "-s", dest="season", help="If set, it will start downloading the from that season."
)
parser.add_argument(
    "-o",
    "--output",
    dest="outputfolder",
    help="If set, it will download all assets to directory provided.",
)
parser.add_argument(
    "--alang",
    "--audio-language",
    dest="audiolang",
    nargs="*",
    help="If set, download only selected audio languages",
    default=[],
)
parser.add_argument(
    "--slang",
    ...
    dest="sublang",
    nargs="*",
    help="If set, download only selected subtitle languages",
    default=[],
)
parser.add_argument(
    "--flang",
    ...
    dest="forcedlang",
    nargs="*",
    help="If set, download only selected forced subtitle languages",
    default=[],
)
parser.add_argument(
    "--license",
    dest="license",
    help="If set, print keys and exit.",
    action="store_true",
)
parser.add_argument(
    "--nv",
    "--no-video",
    dest="novideo",
    help="If set, don't download video",
    action="store_true",
)
parser.add_argument(
    "--na",
    "--no-audio",
    dest="noaudio",
    help="If set, don't download audio",
    action="store_true",
)
parser.add_argument(
    "--ns",
    "--no-subs",
    dest="nosubs",
    help="If set, don't download subs",
    action="store_true",
)
parser.add_argument(
    "--keep",
    dest="keep",
    help="If set, well keep all files after mux, by default all erased.",
    action="store_true",
)
parser.add_argument("--group", "--gr", dest="group", help="Tag.", action="store")
parser.add_argument(
    "--txtkeys", dest="txtkeys", help="If set, read keys from txt.", action="store_true"
)
...
mp4dumptexe = dirPath + "/bin/mp4dump"
...
if Nm3u8DLRE is None:
    print("no executable found for command 'N_m3u8DL-RE'")

proxy_user = {"proxy": "---", "email": "---", "passwd": "---"}
...
global account_info
account_info = {"email": DSNP_EMAIL, "pass": DSNP_PASS}
...
def get_pssh(url):
    ...
    for key in m3u8_obj.keys:
        if (
            key is not None
            and key.keyformat == "urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed"
        ):
...
def getKeyId(mp4_file):
    ...
    data = subprocess.check_output(
        [mp4dumptexe, "--format", "json", "--verbosity", "1", mp4_file]
    )
    ...
    if not os.path.exists(token_file):
        ...
        # ~ check if token will be expired within 10 minutes.
            ...
            try:
                print(
                    "Done: expire in: "
                    + str(int((int(token["expire_date"]) - int(time.time())) / 60))
                    + " min"
                )
            except Exception:
                pass
...
def getWVKeys(pssh, episodename):
    ...
    for key in cdm.get_keys(session_id):
        if key.type == "CONTENT":
            wvkeys.append(f"{key.kid.hex}:{key.key.hex()}")
...
def do_clean(CurrentName):
    ...
    try:
        for pattern in patterns:
            for f in glob.glob(CurrentName + pattern):
                os.remove(f)
    except Exception:
        pass
...
def decryptmedia(KEYS, inputName, outputName):
    if shakaCmd is None:
        ...
        shaka_command = [
            shakaCmd,
            f"in={inputName},stream=video,output={outputName}",
            "--enable_raw_key_decryption",
            "--keys",
            f"key_id={key_id}:key={key}",
        ]
        subprocess.run(shaka_command)

    return True


def vtt2srt(vtt, srt):
    with open(vtt, "r", encoding="utf8") as f:
        subs = f.read()
...
def download_m3u8DL(url, output):
...
def main(episodename, seasonfolder, m3u8Url, SHOW=True):
    ...
    if not args.license:
        if args.customquality:
            ...
            if not int(height) in quality_available:
                print(
                    f"This quality is not available, the available ones are: "
                    + ", ".join(str(x) for x in quality_available)
                    + "."
                )
    ...
    if args.hevc:
        inputVideoBase = episodename + " [" + CurrentHeigh + "p] [HEVC]"
        inputVideo = inputVideoBase + ".mp4"
        ...
        inputVideoBase = episodename + " [" + CurrentHeigh + "p] [HDR]"
        inputVideo = inputVideoBase + ".mp4"
        ...
        inputVideoBase = episodename + " [" + CurrentHeigh + "p]"
        inputVideo = inputVideoBase + ".mp4"
    ...
    if not args.novideo:
        if (
            not os.path.isfile(inputVideo)
            and not os.path.isfile(inputVideo_decrypted)
            and not os.path.isfile(inputVideo_demuxed)
        ):
            print("\nDownloading video...")
            download_m3u8DL(url=videoList["url"], output=inputVideoBase)
        else:
        ...
        if args.audiolang:
            for aud in AudioList:
                if aud["code"] in args.audiolang:
                    AudioEncBase = episodename + " " + aud["code"]
                    AudioEnc = AudioEncBase + ".m4a"
                    ...
                    if not os.path.isfile(AudioDem):
                        if not os.path.isfile(AudioEnc):
                            download_m3u8DL(url=aud["url"], output=AudioEncBase)
                        else:
                            print(AudioEnc + "\ndownloaded previously.")
                    else:
                        print(AudioEnc + "\ndownloaded previously.")
    ...
    if keys_requested:
        ...
        if os.path.isfile(inputVideo_decrypted) and not os.path.isfile(
            inputVideo_demuxed
        ):
            print("\nRemuxing video...")
    ...
    for aud in AudioList:
        AudioEnc = episodename + " " + aud["code"] + ".m4a"
        AudioDem = episodename + " " + aud["code"] + AudioExtension
        ...
        if args.sublang:
            for sub in subtitleList:
                if sub["code"] in args.sublang:
                    subnameBase = episodename + " " + sub["code"]
                    subname = subnameBase + ".srt"
                    if not os.path.isfile(subname):
                        download_m3u8DL(sub["url"], subnameBase)
                    else:
                        print(
                            str(sub["language"])
                            + " - has already been successfully downloaded previously."
                        )
        else:
            for sub in subtitleList:
                subnameBase = episodename + " " + sub["code"]
        ...
        if args.forcedlang:
            for sub in forcedlist:
                if sub["code"] in args.forcedlang:
                    subnameBase = episodename + " " + "forced-" + sub["code"]
                    subname = subnameBase + ".srt"
                    if not os.path.isfile(subname):
                        download_m3u8DL(sub["url"], subnameBase)
                    else:
    ...
    if args.default_audio_mux:
        muxer_defaults.update({"audio": str(args.default_audio_mux)})
    else:
        muxer_defaults.update({"audio": "eng"})
    if args.default_sub_mux:
        muxer_defaults.update({"sub": str(args.default_sub_mux)})
    else:
        muxer_defaults.update({"sub": "eng"})
        ...
        if os.path.isfile(inputVideo_demuxed) and AudioExist:
            ...
            if SHOW == False:
                MKV_Muxer = Muxer(
                    ...
                    mkvmergeexe=mkvmergeCmd,
                )
                MKV_Muxer.DPMuxer()
                # mux(inputVideo_decrypted, MKVOUT1, AudioList, subtitleList)
            else:
    ...
    if not "movies" in dsnpurl and not args.novideo:
        namer.rename(file=MKVOUT2, source="DSNP", group=args.group)
...
if __name__ == "__main__":
    ...
    # if any([x in dsnpurl for x in ['video', 'movies']]):
    if "movies" in dsnpurl:
        print("\nGetting movie metadata & m3u8...")
        dsnp = DSNP(
            dsnpid, AuthorizationToken, "movie", ishdr=ishdr, isuhd=isuhd, ishevc=ishevc
        )
        movie = dsnp.load_playlist()
        url = dsnp.load_info_m3u8_post(
            movie["id"]["mediaId"],
            movie["mediaFormat"],
            movie["playbackUrl"],
            args.customquality,
